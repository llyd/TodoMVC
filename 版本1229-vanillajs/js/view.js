...
(function (window) {
	...
	View.prototype._removeTodo = function (id) {
		var elem = qs('#' + id );
		if (elem) {
			qs(".moreList").removeChild(elem);
		}
	};
...
//修改完todoitem
	View.prototype._editItemDone = function (id, title) {
		...
		qsa('label', listItem).forEach(function (label) {
			label.textContent = title_arr[i];
			i++;
			if(i>title_arr.length){
				label.remove();
			}
		});
        var template = '<label>{{tag}}</label>';

	   	if(i<title_arr.length){
			//添加label，并赋值
			var label = qs('[data-id="' + id + '"]' + ' .view').innerHTML;
		    var labels = '';	  
		    for(;i<title_arr.length;i++){
		    	var template1 = template;
		    	template1 = template1.replace('{{tag}}',title_arr[i]); 	    	
		    	labels = labels + template1;
		    }
		    qs('[data-id="' + id + '"]' + ' .view').innerHTML = label + labels;		
		}

	 	var data = JSON.parse(localStorage[this._todoid]);
		var todos = data.todos;
			for (var i = 0; i < todos.length; i++) {
				if (todos[i].id === id) {
					todos[i].tags = title_arr.slice(1);//每一个todo的tags可改变
					var tags = title_arr.slice(1);
					for (var s = 0; s < tags.length; s++){
						var tag_num = 0;
						for(var num = 0; num<data.tags.length; num++){
							if(data.tags[num] == tags[s]){
								tag_num++;
							}
						}
						if(tag_num <= 0){
						data.tags.push(tags[s]);//没有去重,添加去重??无效
					}
					}
				}
			}
           
			localStorage[this._todoid] = JSON.stringify(data);
...
//修改完title
  View.prototype._editTitleDone = function (id, name) {
	...
	View.prototype.bind = function (event, handler) {
		...
		} else if  (event === 'todoRemove'){
            $delegate(self.$header, '.destroy', 'click', function () {
				handler({id: self._todoid});
			});
		}
	};
